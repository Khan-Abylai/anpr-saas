...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        output0_config_prob = pb_utils.get_output_config_by_name(
        ...
        self.output0_dtype_prob = pb_utils.triton_string_to_numpy(
            output0_config_prob['data_type'])

        eu_sym = {'å': '@', 'ä': '&', 'ć': '!', 'č': '?', 'đ': '%', 'ö': '^', 'ü': '#', 'š': '$', 'ž': '|'}
        eu_add = ''.join(str(key) for key, val in eu_sym.items())

        self.converter = StrLabelConverter(string.digits + string.ascii_lowercase + eu_add)
        self.regions = ['albania', 'andorra', 'austria', 'belgium', 'bosnia', 'bulgaria', 'croatia', 'cyprus', 'czech', 'estonia',
    ...
    def execute(self, requests):
        ...
        output_prob_dtype = self.output0_dtype_prob
        responses = []
        ...
        for request in requests:
            ...
            in_1 = pb_utils.get_input_tensor_by_name(request, "input_cls")

            predictions = torch.from_numpy(in_0.as_numpy())
            cls_predictions = torch.from_numpy(in_1.as_numpy())
            ...
            out_tensor_2 = pb_utils.Tensor("region", np.array(self.regions[cls_idx]).astype(output0_dtype))
